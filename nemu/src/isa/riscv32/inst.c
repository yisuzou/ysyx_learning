...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R,
          R(rd) = (word_t)(((int64_t)(sword_t)src1 * (int64_t)(sword_t)src2) >>
                           32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R,
          R(rd) = (word_t)(((uint64_t)src1 * (uint64_t)src2) >> 32));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu, I,
          R(rd) = src1 < imm); // 所有imm应该进行到操作在上面到宏当中已经完成,
                               // 这里使用的仍然是有符号扩展
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai, I,
          R(rd) = (sword_t)src1 >> (imm & 0x0000001f));
